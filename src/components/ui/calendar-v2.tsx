import * as React from "react"
import { ChevronLeft, ChevronRight } from "lucide-react"
import {
  Chevron as ChevronDayPicker,
  DayPicker,
  MonthsDropdown as MonthsDropdownDayPicker,
  YearsDropdown as YearsDropdownDayPicker,
  useDayPicker,
} from "react-day-picker"

import { cn } from "cn"
import { buttonVariants } from "@/components/ui/button"
import { CalendarDropdown } from "@/components/ui/calendar-dropdown"

export type CalendarV2Props = React.ComponentProps<typeof DayPicker> & {
  selectableStart?: Date
  selectableEnd?: Date
  captionLabelClassName?: string
  dayClassName?: string
  dayButtonClassName?: string
  dropdownsClassName?: string
  footerClassName?: string
  monthClassName?: string
  monthCaptionClassName?: string
  monthGridClassName?: string
  monthsClassName?: string
  navClassName?: string
  buttonNextClassName?: string
  buttonPreviousClassName?: string
  weekClassName?: string
  weekdayClassName?: string
  weekdaysClassName?: string
  rangeEndClassName?: string
  rangeMiddleClassName?: string
  rangeStartClassName?: string
  selectedClassName?: string
  disabledClassName?: string
  hiddenClassName?: string
  outsideClassName?: string
  todayClassName?: string
  selectTriggerClassName?: string
}

function MonthsDropdown(props: React.ComponentProps<typeof MonthsDropdownDayPicker>) {
  const { dayPickerProps, months } = useDayPicker()
  const { selectableStart, selectableEnd, selectTriggerClassName } =
    dayPickerProps as CalendarV2Props

  const displayedYear = months[0]?.date.getFullYear() ?? new Date().getFullYear()
  const rangeStart = selectableStart
    ? { year: selectableStart.getFullYear(), month: selectableStart.getMonth() }
    : null
  const rangeEnd = selectableEnd
    ? { year: selectableEnd.getFullYear(), month: selectableEnd.getMonth() }
    : null

  const options = (props.options ?? []).map((option) => {
    const beforeStart =
      selectableStart && rangeStart
        ? displayedYear < rangeStart.year ||
          (displayedYear === rangeStart.year && option.value < rangeStart.month)
        : false
    const afterEnd =
      selectableEnd && rangeEnd
        ? displayedYear > rangeEnd.year ||
          (displayedYear === rangeEnd.year && option.value > rangeEnd.month)
        : false
    return { ...option, disabled: beforeStart || afterEnd }
  })

  return (
    <CalendarDropdown
      value={props.value === undefined ? undefined : Number(props.value)}
      options={options}
      disabled={props.disabled}
      ariaLabel={props["aria-label"]}
      triggerClassName={selectTriggerClassName}
      onSelect={(value) => {
        props.onChange?.({
          target: { value: String(value) },
        } as React.ChangeEvent<HTMLSelectElement>)
      }}
    />
  )
}

function YearsDropdown(props: React.ComponentProps<typeof YearsDropdownDayPicker>) {
  const { dayPickerProps } = useDayPicker()
  const { selectTriggerClassName } = dayPickerProps as CalendarV2Props

  const options = [...(props.options ?? [])].reverse()

  return (
    <CalendarDropdown
      value={props.value === undefined ? undefined : Number(props.value)}
      options={options}
      disabled={props.disabled}
      ariaLabel={props["aria-label"]}
      triggerClassName={selectTriggerClassName}
      onSelect={(value) => {
        props.onChange?.({
          target: { value: String(value) },
        } as React.ChangeEvent<HTMLSelectElement>)
      }}
    />
  )
}

function CalendarV2({
  className,
  classNames,
  hideNavigation,
  showOutsideDays = true,
  components,
  ...props
}: CalendarV2Props) {
  const _monthsClassName = cn("relative flex flex-col gap-4 sm:flex-row", props.monthsClassName)
  const _monthClassName = cn("relative flex flex-col gap-4", props.monthClassName)
  const _monthCaptionClassName = cn(
    "relative flex h-7 items-center justify-center px-8",
    props.monthCaptionClassName
  )
  const _dropdownsClassName = cn(
    "flex items-center justify-center gap-3",
    hideNavigation ? "w-full" : "",
    props.dropdownsClassName
  )
  const _footerClassName = cn("pt-3 text-sm", props.footerClassName)
  const _weekdaysClassName = cn("flex", props.weekdaysClassName)
  const _weekdayClassName = cn(
    "w-9 text-center text-sm font-normal text-muted-foreground",
    props.weekdayClassName
  )
  const _captionLabelClassName = cn("truncate text-sm font-medium", props.captionLabelClassName)

  const buttonNavClassName = buttonVariants({
    variant: "outline",
    className: "z-10 size-7 bg-transparent p-0 opacity-50 hover:opacity-100 top-0",
  })
  const _buttonNextClassName = cn(buttonNavClassName, "absolute right-0", props.buttonNextClassName)
  const _buttonPreviousClassName = cn(
    buttonNavClassName,
    "absolute left-0",
    props.buttonPreviousClassName
  )
  const _navClassName = cn("absolute flex w-full items-center justify-between", props.navClassName)
  const _monthGridClassName = cn("mx-auto mt-4", props.monthGridClassName)
  const _weekClassName = cn("mt-2 flex w-max items-start", props.weekClassName)
  const _dayClassName = cn(
    "flex size-9 flex-1 items-center justify-center p-0 text-sm",
    props.dayClassName
  )
  const _dayButtonClassName = cn(
    buttonVariants({ variant: "ghost" }),
    "size-9 rounded-md p-0 font-normal transition-none aria-selected:opacity-100",
    props.dayButtonClassName
  )

  const buttonRangeClassName =
    "bg-accent [&>button]:bg-primary [&>button]:text-primary-foreground [&>button]:hover:bg-primary [&>button]:hover:text-primary-foreground"

  const _rangeStartClassName = cn(buttonRangeClassName, "rounded-s-md", props.rangeStartClassName)
  const _rangeEndClassName = cn(buttonRangeClassName, "rounded-e-md", props.rangeEndClassName)
  const _rangeMiddleClassName = cn(
    "bg-accent !text-foreground [&>button]:bg-transparent [&>button]:!text-foreground [&>button]:hover:bg-transparent [&>button]:hover:!text-foreground",
    props.rangeMiddleClassName
  )
  const _selectedClassName = cn(
    "[&>button]:bg-primary [&>button]:text-primary-foreground [&>button]:hover:bg-primary [&>button]:hover:text-primary-foreground",
    props.selectedClassName
  )
  const _todayClassName = cn(
    "[&>button]:bg-accent [&>button]:text-accent-foreground",
    props.todayClassName
  )
  const _outsideClassName = cn(
    "text-muted-foreground opacity-50 aria-selected:bg-accent/50 aria-selected:text-muted-foreground aria-selected:opacity-30",
    props.outsideClassName
  )
  const _disabledClassName = cn("text-muted-foreground opacity-50", props.disabledClassName)
  const _hiddenClassName = cn("invisible flex-1", props.hiddenClassName)

  const Chevron = React.useCallback(
    ({ orientation, ...chevronProps }: React.ComponentProps<typeof ChevronDayPicker>) => {
      const Icon = orientation === "left" ? ChevronLeft : ChevronRight
      return <Icon className="size-4" {...chevronProps} />
    },
    []
  )

  return (
    <DayPicker
      showOutsideDays={showOutsideDays}
      hideNavigation={hideNavigation}
      className={cn("p-3", className)}
      classNames={{
        caption_label: _captionLabelClassName,
        day: _dayClassName,
        day_button: _dayButtonClassName,
        dropdowns: _dropdownsClassName,
        footer: _footerClassName,
        month: _monthClassName,
        month_caption: _monthCaptionClassName,
        month_grid: _monthGridClassName,
        months: _monthsClassName,
        nav: _navClassName,
        button_next: _buttonNextClassName,
        button_previous: _buttonPreviousClassName,
        week: _weekClassName,
        weekday: _weekdayClassName,
        weekdays: _weekdaysClassName,
        range_end: _rangeEndClassName,
        range_middle: _rangeMiddleClassName,
        range_start: _rangeStartClassName,
        selected: _selectedClassName,
        disabled: _disabledClassName,
        hidden: _hiddenClassName,
        outside: _outsideClassName,
        today: _todayClassName,
        ...classNames,
      }}
      components={{
        Chevron,
        MonthsDropdown,
        YearsDropdown,
        ...components,
      }}
      {...props}
    />
  )
}

CalendarV2.displayName = "CalendarV2"

export { CalendarV2 }
